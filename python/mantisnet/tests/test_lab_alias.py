...
def test_single_stone_position_reports_the_all_empty_alias_group():
    """One stone leaves 186 cells with the same all-EMPTY row signature."""
    pos = hexo_py.Position.replay([(0, 0)])
    report = alias_report([pos])
    assert report["positions"] == 1
    assert report["baked"]["legal_actions"] == 216
    assert report["baked"]["max_alias_group"] == 186
...
def test_alias_counts_are_d6_invariant():
    ...
    for moves in _GAMES:
        ...
        assert base["baked"] == turned["baked"]
