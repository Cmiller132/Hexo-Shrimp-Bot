...
def test_refuses_mismatched_score_and_value_shapes():
    with pytest.raises(ValueError, match="q_score and q_value"):
        improved_policy(
            torch.zeros(2), torch.zeros(2), torch.zeros(3),
            torch.tensor([0, 2]), 0.1, 0.01,
        )
...
def test_expectations_stay_inside_the_q_range_when_every_action_is_lost():
    ...
    out = improved_policy(torch.zeros(4), q, q, offsets, 1e9, 1e9)
