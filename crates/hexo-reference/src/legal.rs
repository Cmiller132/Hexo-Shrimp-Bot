...
//!
//! VENDORED CHANGE: `ahash::AHashSet` swapped for `std::collections::HashSet`
//! (a hasher swap; enumeration order comes from the sorted `ordered` vec, not
//! from the set). Unit tests removed.
...
pub struct LegalMoveStore {
    membership: HashSet<PackedCoord>,
