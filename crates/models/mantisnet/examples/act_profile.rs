...
//!
//! Run the direct packed-and-planned hot path with `--packed`, or two
//! simultaneous calls (the fitloop prefetch shape) with `--concurrent`.
//! Add `--action-relevant` to either mode to switch the window scope and
//! `--count N` to choose the position count (default 64).

use hexo_model_mantisnet::act_encoder::{
    self, ActBuilderConfig, CellScope, D6RelationMode, WindowScope,
};
use hexo_model_mantisnet::act_plans;
use std::fs;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let count = arguments
        ...
        .map_or(Ok(64usize), |pair| pair[1].parse())?;
    if count == 0 {
            ...
            for _ in 0..2 {
                ...
                scope.spawn(move || {
                    while requested.recv().is_ok() {
                        barrier.wait();
                        let batch = act_plans::build_planned_batch_prefixes(games, plies, cfg);
    ...
    if arguments.iter().any(|argument| argument == "--packed") {
        for iteration in 0..5 {
            let started = Instant::now();
            let packed = act_encoder::build_packed_batch_prefixes(&games, &plies, &cfg)?;
            let built = started.elapsed();
            let plan_started = Instant::now();
            let plans = act_plans::build_plan_arrays(&packed, &cfg)?;
            let planned = plan_started.elapsed();
            let elapsed = started.elapsed();
            println!(
                "iteration={iteration} packed_planned_ms={:.3} build_ms={:.3} plan_ms={:.3} pos_s={:.1} radius={} action_plan_rows={}",
                elapsed.as_secs_f64() * 1e3,
                built.as_secs_f64() * 1e3,
                planned.as_secs_f64() * 1e3,
                count as f64 / elapsed.as_secs_f64(),
                packed.radius_src.len(),
