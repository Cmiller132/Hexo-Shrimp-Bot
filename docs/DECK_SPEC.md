...
The deck is the run's face: a LAN-served dashboard over the telemetry a run
already writes, plus the play/inspection surfaces the checkpoints already
support. It ships in two pieces — `mantisnet.deck`, a FastAPI service inside
the existing Python project, and `frontend/`, a Vite + React single-page app
— and runs only in the Docker environment (`docker/`), where every future
training run also lives. There is no Windows-native serving path.

The visual language, layout, and screen inventory are the existing draft in
`frontend/` (four screens: Play, Game history, Live run, Model lab, plus the
command palette and compare tray). That draft is mock-driven; this spec is
the contract for making every number on it real. Where the draft and this
...
- **`telemetry.db` is read-only to the deck's query paths.** The schema and
  ...
  `sealbot.record_match` writer path with a `busy_timeout`, exactly as the
  offline CLI sweep already does.
- **The training loop is not this project's to edit.** The deck consumes the
  run driver's artifacts — `telemetry.db`, `metrics.jsonl`, `config.json`,
  `invocations.jsonl`, checkpoints, `status.json`, `resume-console` logs —
  and controls a run only through the child-process boundary and the two
  sentinel files the driver honors (below). No edits to `run.py`,
  `train.py`, `selfplay.py`, `model.py`, or anything else under
  `mantisnet/` outside the new `mantisnet/deck/` package.
- **No auth, LAN-trusted.** The deck binds `0.0.0.0` and every control is
  live. This is a deliberate owner decision for a private network; do not
  add token machinery.
- **Fail loudly.** A missing run directory, an unreadable database, a
  version-mismatched checkpoint, a dead child process — all are errors
  surfaced to the client as structured HTTP errors and to the event stream,
  never silently empty panels.
- **One implementation per job.** The draft's Cloudflare/vinext/wrangler
  layer is replaced (below), not kept alongside.
...
### The run driver's side of the contract (already landing on `mantisnet`)

These exist (or are landing in parallel on the `mantisnet` branch) and the
deck builds against their formats; it does not implement them.
...
### `mantisnet.deck` (new package, `python/mantisnet/mantisnet/deck/`)

FastAPI app plus the non-HTTP guts, structured however reads best — the
seams that matter:
...
- **Run registry.** A run is a directory under `runs/` containing
  ...
  which work regardless of who launched the run, and are the *preferred*
  stop mechanism everywhere; killing is for hung processes.
...
### `frontend/` (folded into this repo from the draft)

- **Stack change (normative):** plain Vite + React 19 + TypeScript +
  Tailwind v4 + lucide-react, client-only SPA. `next`/`vinext`/`wrangler`/
  RSC machinery is removed entirely — `index.html` + a mount point replaces
  `app/layout.tsx`/`page.tsx`; `ConsoleApp.tsx` and `globals.css` carry
  over as the design source. `npm run build` emits `frontend/dist`;
  `npm run dev` proxies `/api` and the SSE path to :8000. Keep the
  rendered-shell test working against the Vite build.
- **Decompose as you wire.** 1,200 lines of mock JSX in one file was right
  for a draft; wired screens split into modules (per screen + shared board/
  chart/API-client pieces). Preserve the visual design; replace every mock
  constant with live data. A panel whose datum has no backend yet does not
  ship half-alive: it either renders real data or is removed.
- **Screens, wired:**
  - **Live run** — run selector = real run list (state-badged). Status
    ...
    draft, plus per-iteration seconds); eval panel from the latest
    `eval_matches` row (CI, Elo, seat split are all real columns); losses/
  ...
  - **Game history** — table = `search_games` (kind, winner, length,
    iteration range, capped, pagination); filters real; opening column via
  ...
  - **Play** — everything through play sessions: human seats click the
    real board (engine-checked), bot seats reply on their mode; overlays
    (πθ / Q / π′ / rank) and the candidate list from `inspect_position`,
    colored over the real legal set; the KLENT position read (v̂, KL,
    ...
    The draft's "quick suites" reduce to what exists: cross-play launcher
    and a SealBot set; the πθ/Q-disagreement suite is saved lab probes.
...
- **Realtime behavior:** SSE-driven refresh on the live screen; history/
  lab/play refetch on navigation and on demand. The draft's pause-telemetry
  toggle pauses the SSE subscription client-side.
...
- `deck`: runs `uvicorn` on :8000 (published), GPU visible, entrypoint
  ...
  are missing, then serves. Restart unless-stopped: the deck is up whenever
  Docker is.
...
`runs/` stays on the repo bind mount (owner decision 2026-07-28) — one home
for all runs, visible from Windows.

## Definition of done

From `python/mantisnet` in the container:
1. `pytest tests/ -q` green, including new `tests/test_deck*.py` covering:
   run registry states, lifecycle sentinels (against a scripted fake run
   process), every query endpoint against a real heuristic-run telemetry
   database (the pipeline tests already build these on CPU), SSE event
...
2. `npm run build` and the rendered-shell test pass in `frontend/`.
3. `docker compose up deck` serves the SPA and API on :8000; every panel on
   all four screens renders real data against a live heuristic run launched
   through the API (no mock constants reachable in the shipped bundle).
4. `python/mantisnet/README.md` gains a Deck section (module map + the
   run-artifact contract); `frontend/README.md` rewritten for the real app;
   `docker/README.md` gains the two services.
