...
| R1 screen v2: `cell_structure`, `global_magnitude`, `cell_node_scope=uncovered`, `orbit_vectors` | First round under screen v2: 3 seeds × the full 1M-position train split × 4 epochs per cell, EMA-scored on the paired val 100k; fixture = 6 seeds of the live configuration on the integration tree (critic CE 0.6183 ±0.0031, policy NLL 1.8595 ±0.0218, top-1 48.42 ±0.52 pp, ρ(critic, policy) +0.18, 0/6 pathological); composite S = (2·z_c + z_p)/√(5+4ρ), keep bar S ≥ 2 pooled | `cell_structure` S +1.44 (seeds +0.1/−0.2/+3.0): policy NLL −0.0597 (z_p +3.87), top-1 +1.15 pp, critic CE +0.0005 (z_c −0.22). `global_magnitude` S −2.99 (−3.6/+0.4/−2.7): policy NLL +0.0832, top-1 −1.72 pp. Both knobs together S +1.22 (+0.3/+1.3/+0.8). `cell_node_scope=uncovered` S −5.62 (−1.8/−3.5/−5.7): policy NLL +0.1535, top-1 −2.20 pp. `orbit_vectors` S +0.93 (+0.8/+0.5/+0.5). 0/15 cells pathological | Train-throughput ratio against the fixture: `cell_structure` 0.871, `global_magnitude` 0.980, both 0.912, `uncovered` 1.047, `orbit_vectors` 0.963 | No arm met the keep bar. `global_magnitude` and `orbit_vectors` removed 2026-08-21 (reverts c575ed9 and 676e0e5; feature branches kept as provenance); `cell_node_scope` default stays `all`. PROVISIONAL, owner decision list: `cell_structure` is retained as an off-default knob — its gain is policy-only, which the composite's 2:1 critic weighting discounts — and `uncovered` scope is negative on quality but 4.7% faster |
