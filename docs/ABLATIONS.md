...
### Fit-path VRAM: recompute-in-backward trunk (owner-directed, 2026-08-17)

| Field | Record |
| --- | --- |
| Directive | Shrink fit VRAM ≥30% (ideally >50%) at kept-or-better speed, at the model level — no budget knobs. Baseline: the step56 gate's base arm, 6.280 GiB / 1041.1 samples/s median on the pinned instrument (lean budgets 2M/125k). |
| Commits | `9848a14` (cell/broadcast attention stop saving out/m/l), `468ad4d` (cell stage + window-latent cycle wrapped in non-reentrant checkpointing), `31869c6` (two-pass dq sweeps form delta in-kernel; no backward-side forward relaunch), `c034917` (selective policy: the four attention ops' outputs saved, everything else replays; vcls gradient kernel at 16-edge tiles). `d11b650` reverts a measured no-op (explicit bf16 casts moved nothing: the fp32 peak residents are the residual streams, which only recompute can remove). |
| Attribution | Peak live set at baseline: §5.1b cell stage 52.6% (3.40 GiB), §5.4 window latents 18.0% (1.16 GiB) — dozens of (N, H) saves at 61 MiB fp32 / 30.5 MiB bf16 per tensor at the 125k cell budget. Allocated-vs-reserved gap 6.55/10.48 GiB at baseline collapses to 3.6/4.8 with the smaller live set. |
| Dead ends (measured) | Head-fused kernel programs: 766 samples/s vs 924 — the 3-D gather tiles' register pressure beats the shared index loads on this part; reverted to per-head programs. Blanket recompute without the selective policy: every op forward ran twice, −2.1% at 3.213 GiB. |
| Result | **3.538 GiB (−43.7%) at 1034.3 samples/s** — same-session A/B (the instrument's inter-chunk overhead drifted across the night, so the honest comparison re-benched the pre-change tree in the same session: baseline 1046.8/1048.3/1050.5, work tree 1030.8/1037.8). Chunk compute is at parity or better: steady chunk-ms median 143.3 vs 144.0. The −1.3% sps median is inter-chunk: the synchronous pinned-host→device batch transfer (~3% of device time) is the identified recovery lever, deliberately not attempted overnight under a run launch — a stream-ordering bug there corrupts gradients silently. |
| Exactness | Execution policy only: no parameter, format, or math change. Recompute replays deterministic kernels on identical inputs, so gradients are bit-identical to saved-activation execution; the delta regrouping (per-edge sum in place of go·out) changes gradient rounding only, applied identically to kernels and the parity references. |
| Verification | full pytest **466 passed, 3 skipped** and `cargo xtask verify` all gates green at `c034917` + the launch-script commit; CUDA parity and bitwise-determinism suites green on the rewritten kernels; no fallback shapes recorded. |
